function [im,imD] = ReadKLBs(dirIn,datasetName)
    ...
    imList = dir(fullfile(root,'*.klb'));
    
    [datasetName,chans,cams,stacks,iter,wavelengths,secs,fileSuffixs] = LLSM.ParseFileNames(root,'klb');
    ...
    % pre-allocate memory
    im1 = MicroscopeData.KLB.readKLBstack(fullfile(root,imList(1).name));
    ...
    for i=1:length(imList)
        ...
        curIm = MicroscopeData.KLB.readKLBstack(fullfile(root,imList(i).name));
