...
itk::SmartPointer<kImageType> SubtractConstantClamped(kImageType img, kPixelType c)
{
    ...
    subtract_filter->SetConstant2(100.0 / std::numeric_limits<unsigned short>::max());
...
itk::SmartPointer<kImageType> SubtractConstant(kImageType img, kPixelType c)
{
    using SubtractImageFilterType = itk::SubtractImageFilter<kImageType, kImageType, kImageType>;
    SubtractImageFilterType::Pointer subtract_filter = SubtractImageFilterType::New();
    subtract_filter->SetInput(img);
